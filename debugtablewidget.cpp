...
void
DebugTableWidget::initializeMemoryWindow(const QList<RuQPlainTextEdit::Watch>& watches) {
	if (type == memoryTable) {
...
void
DebugTableWidget::setValuesFromDebugger(QList<Debugger::memoryInfo> watches) //watches
{
...
void
DebugTableWidget::setValuesFromDebugger(QList<Debugger::registersInfo> registers) //registers
{
...
void
DebugTableWidget::changeMemoryWindow(int row, int column) {
...
void
DebugTableWidget::changeVariableValue(const QString& value, int rowNumber, bool isValid) {
	if (type == memoryTable) {
		if (isValid)
...
void
DebugTableWidget::deleteVariable() {
...
void
DebugTableWidget::addVariable(const QString& variableName, int rowNumber) {
	empty = false;
	if (type == memoryTable) {
		if (rowNumber == -1)
...
void
DebugTableWidget::addVariable(const RuQPlainTextEdit::Watch& variable, int rowNumber) {
	empty = false;
	if (type == memoryTable) {
		if (rowNumber == -1)
			rowNumber = rowCount() - 1;
...
void
DebugTableWidget::addRegister(const QString& name, const QString& hexValue, const QString& decValue, int rowNumber) {
	empty = false;
	if (type == registersTable) {
		if (item(rowNumber, 2)) {
...
bool
DebugTableWidget::isEmpty() {
	return empty;
}

void
DebugTableWidget::closeEvent(QCloseEvent*) {
	emit closeSignal();
}

void
DebugTableWidget::mousePressEvent(QMouseEvent* event) {
...
void
DebugTableWidget::keyPressEvent(QKeyEvent* event) {
