...
NASM::NASM(bool x86, QObject* parent) : GccBasedAssembler(x86, parent) {}
...
void NASM::parseLstFile(QFile& lst, QString& programFilePath,
...
QString NASM::getAssemblerOptions() {
  QString options;
#ifdef Q_OS_WIN32
  if (isx86()) options = "-f win32 $SOURCE$ -l $LSTOUTPUT$ -o $PROGRAM.OBJ$";
  else
    options = "-f win64 $SOURCE$ -l $LSTOUTPUT$ -o $PROGRAM.OBJ$";
#else
  if (isx86()) options = "-f elf32 $SOURCE$ -l $LSTOUTPUT$ -o $PROGRAM.OBJ$";
  else
