...
GAS::GAS(bool x86, QObject* parent) : GccBasedAssembler(x86, parent) {}

QString GAS::getAssemblerPath() {
#ifdef Q_OS_WIN32
  if (isx86()) return Common::applicationDataPath() + "/GAS/32/as.exe";
  else
...
quint64 GAS::getMainOffset(QFile& lst, QString entryLabel) {
  ...
  while (!lstStream.atEnd()) {
    ...
    if (flag) {
      ...
      if (line.indexOf(mainLabel) != -1) flag = true;
    }
  }
  return -1;
}

void GAS::parseLstFile(QFile& lst, QString& programFilePath,
  ...
  while (!lstStream.atEnd()) {
    ...
    if (line.indexOf(sectionTextRegExp) != -1) {
      inTextSection = true;
               ...
               line.indexOf(sectionBssRegExp) != -1) {
      inTextSection = false;
    }
    if (line.length() <= 19) {  // omit strings with data only
      // if in list : line number, address, data and it is all (without instruction) -
...
void GAS::putDebugString(CodeEditor* code) {
  // add : mov ebp, esp for making frame for correct debugging if this code has not been
  // added yet
    ...
    if (intelIndex == -1 || attIndex > intelIndex) {  // AT&T syntax
      if (isx86()) {
        if (code->toPlainText().indexOf(
                QRegExp("\\s+[Mm][Oo][Vv][Ll] +%?[Ee][Ss][Pp] *, *%?[Ee][Bb][Pp]"),
                index + 1) != index + 1) {
        ...
        if (code->toPlainText().indexOf(
                QRegExp("\\s+[Mm][Oo][Vv][Qq] +%?[Rr][Ss][Pp] *, *%?[Rr][Bb][Pp]"),
                index + 1) != index + 1) {
          QTextCursor cursor = code->textCursor();
          cursor.movePosition(QTextCursor::Start);
          cursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor, index + 1);
      ...
      if (isx86()) {
        if (code->toPlainText().indexOf(
                QRegExp("\\s+[Mm][Oo][Vv] +%?[Ee][Bb][Pp] *, *%?[Ee][Ss][Pp]"),
                index + 1) != index + 1) {
          QTextCursor cursor = code->textCursor();
          cursor.movePosition(QTextCursor::Start);
          cursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor, index + 1);
        ...
        if (code->toPlainText().indexOf(
                QRegExp("\\s+[Mm][Oo][Vv] +%?[Rr][Bb][Pp] *, *%?[Rr][Ss][Pp]"),
                index + 1) != index + 1) {
          QTextCursor cursor = code->textCursor();
          cursor.movePosition(QTextCursor::Start);
          cursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor, index + 1);
...
QString GAS::getAssemblerOptions() {
  QString options;
  if (isx86()) options = "$SOURCE$ -o $PROGRAM.OBJ$ --32 -a=$LSTOUTPUT$";
  else
