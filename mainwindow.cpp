...
MainWindow::MainWindow(const QStringList& args, QWidget* parent)
  ...
  for (int i = 1; i < args.size(); i++) openFile(args[i]);
}

void MainWindow::enableOrDisableObjectFilenameLabel(int disableCurrentDirCheckboxState) {
  bool enabled = (disableCurrentDirCheckboxState == Qt::Unchecked);
  settingsUi.objectFileNameEdit->setEnabled(enabled);
}
...
void MainWindow::initUi() {
  ...
  setWindowState(
      windowState() |
      (Qt::WindowState) settings.value("maximized", (int) Qt::WindowNoState).toInt());
  settings.endGroup();
  ...
  connect(getStartedWidget->prevSessionButton, SIGNAL(clicked()), this,
          SLOT(restorePrevSession()));
  ...
  splitter->setSizes(QList<int>()
                     << splitter->size().height() - compilerOutSize << compilerOutSize);
  //! Restore previous session if it in settings
  if (settings.value("startwindow", 0).toInt() == 1) restorePrevSession(true);
}
...
void MainWindow::createActions() {
  QSettings keySettings(Common::applicationDataPath() + "/keys.ini",
                        QSettings::IniFormat);
  ...
  if (key == "default") key = "Ctrl+Shift+E";
  ...
  connect(debugShowMemoryAction, SIGNAL(toggled(bool)), this, SLOT(debugShowMemory()),
          Qt::QueuedConnection);
...
void MainWindow::refreshEditMenu() {
  ...
  if (prevCodeEditor) {
    disconnect(commentAction, SIGNAL(triggered()), prevCodeEditor,
               SLOT(commentSelectedCode()));
    disconnect(uncommentAction, SIGNAL(triggered()), prevCodeEditor,
               SLOT(uncommentSelectedCode()));
    ...
    disconnect(prevCodeEditor, SIGNAL(undoAvailable(bool)), undoAction,
               SLOT(setEnabled(bool)));
    disconnect(prevCodeEditor, SIGNAL(redoAvailable(bool)), redoAction,
               SLOT(setEnabled(bool)));
  }
  ...
  if (tabs->currentIndex() != -1) {
    ...
    connect(uncommentAction, SIGNAL(triggered()), codeEditor,
            SLOT(uncommentSelectedCode()));
    ...
    connect(codeEditor, SIGNAL(copyAvailable(bool)), commentAction,
            SLOT(setEnabled(bool)));
    connect(codeEditor, SIGNAL(copyAvailable(bool)), uncommentAction,
            SLOT(setEnabled(bool)));
    connect(codeEditor, SIGNAL(copyAvailable(bool)), deleteAction,
            SLOT(setEnabled(bool)));
  }
...
void MainWindow::newFile() {
  ...
  connect(tab->code, SIGNAL(modificationChanged(bool)), this,
          SLOT(changeCurrentSavedState(bool)));
...
void MainWindow::openFile() {
  QString fileName =
      QFileDialog::getOpenFileName(this, tr("Open file"), saveOpenDirectory,
...
void MainWindow::setTabPath(const QString& filePath, int index) {
  ...
  int i;
  if ((i = filePath.lastIndexOf('/')) == -1) i = filePath.lastIndexOf('\\');
...
void MainWindow::closeFile() { deleteTab(tabs->currentIndex()); }

bool MainWindow::saveFile(int index, bool openSaveAs) {
  Tab* tab;
  if (index == -1) tab = (Tab*) tabs->currentWidget();
  else
...
bool MainWindow::saveAsFile(int index) {
  QString fileName =
      QFileDialog::getSaveFileName(this, tr("Save file"), saveOpenDirectory,
                                   tr("Assembler source files (*.asm);;All files (*)"));
  if (fileName.isEmpty()) {
    return false;
  }
...
bool MainWindow::closeApp() {
  ...
  if (tabsCount > 0) {
    ...
    for (int i = 0; i < tabsCount; i++) {
      ...
      if (!deleteTab(tabs->currentIndex(), true)) return false;
    }
  }
  writeSettings();
...
bool MainWindow::deleteTab(int index, bool saveFileName) {
  ...
  if (okToContinue(index)) {
    if (debugger && index == tabs->currentIndex()) debugExit();
    if (tabs->count() == 1)
      //! Get started
    ...
    if (tabForDeleting->code == prevCodeEditor) prevCodeEditor = 0;
    return true;
  }
  return false;
}
...
bool MainWindow::okToContinue(int index) {
  Tab* deletingTab;
  if (index == -1) deletingTab = (Tab*) tabs->currentWidget();
  else
...
void MainWindow::buildProgram(bool debugMode) {
  ...
  if (!QFile::exists(applicationDataPath())) {
    printLogWithTime(
        tr("Error! Program directory does not exist. Please reinstall the program.") +
            '\n',
        Qt::red);
    QMessageBox::critical(
        0, tr("Error!"),
        tr("Directory NASM does not exist. Please reinstall the program."));
    return;
  }
  ...
  replaceSOURCE = currentTab->getCurrentFilePath();
  QFileInfo sourceFileInfo = QFileInfo(replaceSOURCE);
  replaceWORKSPACE_ROOT = sourceFileInfo.absoluteDir().absolutePath();

  bool currentDir = settings.value("currentdir", false).toBool();
  if (currentDir) {
    QString buildDirPath = replaceWORKSPACE_ROOT + "/build";
    outputCOMPILER_ERROR = buildDirPath + "/compilererror.txt";
    outputLINKER_ERROR = buildDirPath + "/linkererror.txt";
    replaceLSTOUTPUT = buildDirPath + '/' + sourceFileInfo.baseName() + ".lst";
    replacePROGRAM = buildDirPath + '/' + sourceFileInfo.baseName() + ".exe";
    replacePROGRAM_OBJ = buildDirPath + '/' + sourceFileInfo.baseName() + ".obj";

    // Create build directory if necessary
    QDir buildDir = QDir(buildDirPath);
    if (!buildDir.exists()) buildDir.mkdir(buildDirPath);
  } else {
    replaceSOURCE = Common::pathInTemp("program.asm");
    outputCOMPILER_ERROR = Common::pathInTemp("compilererror.txt");
    outputLINKER_ERROR = Common::pathInTemp("linkererror.txt");
    replaceLSTOUTPUT = Common::pathInTemp("program.lst");
    replacePROGRAM = Common::pathInTemp("SASMprog.exe");
    replacePROGRAM_OBJ =
        Common::pathInTemp(settings.value("objectfilename", "program.obj").toString());
  }

  currentTab->saveCodeToFile(
      replaceSOURCE, assembler, false,
      debugMode && settings.value("insertdebugstring", true).toBool());
  ...
  while (!QFile::exists(replaceSOURCE)) {
  }
  ...
  if (currentDir) {
    assemblerProcess.setWorkingDirectory(replaceWORKSPACE_ROOT);
  } else {
    assemblerProcess.setWorkingDirectory(applicationDataPath() + "/include");
  }

  assemblerOutput = outputCOMPILER_ERROR;
  assemblerArguments.replaceInStrings("$SOURCE$", replaceSOURCE);
  assemblerArguments.replaceInStrings("$LSTOUTPUT$", replaceLSTOUTPUT);
  assemblerArguments.replaceInStrings("$PROGRAM$", replacePROGRAM);
  assemblerArguments.replaceInStrings("$PROGRAM.OBJ$", replacePROGRAM_OBJ);
  assemblerArguments.replaceInStrings("$WORKSPACE_ROOT$", replaceWORKSPACE_ROOT);
...
#ifdef Q_OS_WIN32
  QString linker =
      settings.value("linkerpath", applicationDataPath() + "/MinGW/bin/gcc.exe")
          .toString();
  ...
  gccMArguments << "-x"
                << "c" << Common::pathInTemp("macro.c") << "-c"
                << "-g"
                << "-o" << stdioMacros;
  if (settings.value("mode", QString("x86")).toString() == "x86") gccMArguments << "-m32";
  else
    gccMArguments << "-m64";
  ...
  if (!disableLinking) {
    ...
    linkerArguments.replaceInStrings("$PROGRAM.OBJ$", replacePROGRAM_OBJ);
    linkerArguments.replaceInStrings("$PROGRAM$", replacePROGRAM);
    linkerArguments.replaceInStrings("$SOURCE$", replaceSOURCE);
    linkerArguments.replaceInStrings("$LSTOUTPUT$", replaceLSTOUTPUT);
    linkerArguments.replaceInStrings("$WORKSPACE_ROOT$", replaceWORKSPACE_ROOT);

    QProcess linkerProcess;
    linkerProcess.setStandardOutputFile(outputLINKER_ERROR);
    linkerProcess.setStandardErrorFile(outputLINKER_ERROR, QIODevice::Append);
  ...
  if (settings.value("assembler", QString("NASM")).toString() == "FASM" &&
      logText.count(QChar('\n')) == 2 && logText.contains(QRegExp(" bytes\\.")))
    logText.clear();
  ...
  bool builded;
  if (QFile::exists(replacePROGRAM)) builded = true;
  else
    builded = false;
  if (!builded) {
    ...
    if (!disableLinking) {
      logFile.setFileName(outputLINKER_ERROR);
      logFile.open(QIODevice::ReadOnly);
    ...
    //! QMessageBox::critical(0, tr("Warning!"), tr("Errors have occurred in the
    //! build!"));
  } else {
...
void MainWindow::runProgram() {
  ...
  if (inputFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    args = in.readAll().split(' ');
  }

  //! Run program in code directory
  runProcess->setWorkingDirectory(replaceWORKSPACE_ROOT);
  ...
  runProcess->start(replacePROGRAM, args);

  timer->start(100);
  // Previous - connect(timer, SIGNAL(timeout()), this, SLOT(testStopOfProgram()));
}

void MainWindow::startCountProgramTime() { programExecutionTime.start(); }
...
void MainWindow::runExeProgram() {
  ...
  QString program = replacePROGRAM;
...
void MainWindow::debug() {
  ...
  if (!debugger) {
    ...
    debugger = new Debugger(compilerOut, replacePROGRAM, replaceLSTOUTPUT, replaceSOURCE,
                            replaceWORKSPACE_ROOT, inputPath, assembler);
    ...
    connect(code, SIGNAL(addWatchSignal(const RuQPlainTextEdit::Watch&)), this,
            SLOT(setShowMemoryToChecked(RuQPlainTextEdit::Watch)));
...
void MainWindow::changeDebugActionToStart() {
  ...
  debugAction->setIcon(QIcon(":/images/continue.png"));
  debugAction->setShortcut(debugKey);
  if (!debuggerWasStarted) debuggerWasStarted = true;
  else {
    debugShowRegisters();
    debugShowMemory();
  }
...
void MainWindow::debugShowMemory() {
  if (debugShowMemoryAction->isChecked()) {
    if (!memoryWindow) {
      ...
      connect(memoryWindow, SIGNAL(closeSignal()), this,
              SLOT(setShowMemoryToUnchecked()));
      connect(memoryWindow, SIGNAL(debugShowMemory()), this, SLOT(debugShowMemory()));
      CodeEditor* code = ((Tab*) tabs->currentWidget())->code;
      connect(code, SIGNAL(addWatchSignal(const RuQPlainTextEdit::Watch&)), memoryWindow,
              SLOT(addVariable(const RuQPlainTextEdit::Watch&)));
      connect(debugger, SIGNAL(printMemory(QList<Debugger::memoryInfo>)), memoryWindow,
              SLOT(setValuesFromDebugger(QList<Debugger::memoryInfo>)));

      memoryDock->setWidget(memoryWindow);
      memoryDock->setFeatures(QDockWidget::DockWidgetFloatable |
      ...
      if (registersDock) registersDock->show();
      if (memoryDock) memoryDock->show();
    ...
    if (debugger->isStopped()) {
      ...
      for (int i = 0; i < memoryWindow->rowCount() - 1; i++) {
        if (memoryWindow->cellWidget(i, 2)) {
          WatchSettingsWidget* settings =
              (WatchSettingsWidget*) memoryWindow->cellWidget(i, 2);
          ...
          if (ok && arraySize > 0) watchAsArray = "[" + QString::number(arraySize) + "]";
      ...
      if (memoryWindow->rowCount() > 1 && locker.tryLock()) {
        connect(debugger, SIGNAL(printMemory(QList<Debugger::memoryInfo>)), &locker,
                SLOT(unlock()), Qt::UniqueConnection);
        connect(debugger, SIGNAL(finished()), &locker, SLOT(unlock()),
                Qt::UniqueConnection);
      }
    }
...
void MainWindow::saveWatches(DebugTableWidget* table) {
  ...
  for (int i = 0; i < table->rowCount() - 1; i++) {
    ...
    if (!ok) watch.arraySize = 0;
    watches.append(watch);
  }
}

void MainWindow::setShowMemoryToUnchecked() { debugShowMemoryAction->setChecked(false); }
...
void MainWindow::debugShowRegisters() {
  if (debugShowRegistersAction->isChecked()) {
    if (!registersWindow) {
      ...
      connect(registersWindow, SIGNAL(closeSignal()), this,
              SLOT(setShowRegistersToUnchecked()));
      connect(debugger, SIGNAL(printRegisters(QList<Debugger::registersInfo>)),
              registersWindow,
              SLOT(setValuesFromDebugger(QList<Debugger::registersInfo>)));

      registersDock->setWidget(registersWindow);
      registersDock->setFeatures(QDockWidget::DockWidgetFloatable |
                                 QDockWidget::DockWidgetMovable);
    ...
    if (debugger->isStopped() && locker.tryLock()) {
      connect(debugger, SIGNAL(printRegisters(QList<Debugger::registersInfo>)), &locker,
              SLOT(unlock()), Qt::UniqueConnection);
      connect(debugger, SIGNAL(finished()), &locker, SLOT(unlock()),
              Qt::UniqueConnection);
      if (settings.value("allregisters", false).toBool())
...
void MainWindow::debugExit() {
  ...
  CodeEditor* code = ((Tab*) tabs->currentWidget())->code;
  disconnect(code, SIGNAL(addWatchSignal(const RuQPlainTextEdit::Watch&)), this,
             SLOT(setShowMemoryToChecked(RuQPlainTextEdit::Watch)));
  disconnect(debugger, SIGNAL(printLog(QString, QColor)), this,
             SLOT(printLog(QString, QColor)));
...
void MainWindow::showAnyCommandWidget() {
  ...
  connect(debugAnyCommandWidget, SIGNAL(performCommand(QString, bool)), this,
          SLOT(debugRunCommand(QString, bool)));
}

void MainWindow::closeAnyCommandWidget() {
  disconnect(debugAnyCommandWidget, SIGNAL(performCommand(QString, bool)), this,
             SLOT(debugRunCommand(QString, bool)));
  debugAnyCommandWidget->close();
}
...
void MainWindow::debugRunCommand(QString command, bool print) {
  static SignalLocker locker;
  if (debugger->isStopped() && locker.tryLock()) {
    connect(debugger, SIGNAL(printLog(QString, QColor)), &locker, SLOT(unlock()),
            Qt::UniqueConnection);
    connect(debugger, SIGNAL(finished()), &locker, SLOT(unlock()), Qt::UniqueConnection);
    printLog("> " + command + "\n", QColor(32, 71, 247));
    if (print) debugger->doInput("p " + command + "\n", anyAction);
    else
...
void MainWindow::find() {
  if (!findDialog) {
    ...
    connect(findDialog,
            SIGNAL(findNext(QString, Qt::CaseSensitivity, bool, bool, QString)), this,
            SLOT(findNext(QString, Qt::CaseSensitivity, bool, bool, QString)));
  }
  CodeEditor* code = ((Tab*) tabs->currentWidget())->code;
...
void MainWindow::findNext(const QString& pattern, Qt::CaseSensitivity cs, bool all,
  ...
  if (pattern.isEmpty()) {
    for (int i = 0; i < tabs->count(); i++) {
      ...
      if (code->debugMode) code->highlightDebugLine(code->currentDebugLine);
    }
  } else {
    // If true, find all
    if (all) {
      for (int i = 0; i < tabs->count(); i++) {
        ...
        while (!newCursor.isNull() && !newCursor.atEnd()) {
          if (cs == Qt::CaseSensitive)
            newCursor =
                document->find(pattern, newCursor, QTextDocument::FindCaseSensitively);
          else
...
void MainWindow::openSettings() {
  ...
  if (!settingsWindow) {
    ...
    connect(settingsUi.buttonBox->button(QDialogButtonBox::Ok), SIGNAL(clicked()), this,
            SLOT(exitSettings()));
    connect(settingsUi.buttonBox->button(QDialogButtonBox::Apply), SIGNAL(clicked()),
            this, SLOT(saveSettings()));
    connect(settingsUi.buttonBox->button(QDialogButtonBox::Cancel), SIGNAL(clicked()),
            this, SLOT(restoreSettingsAndExit()));
    connect(settingsUi.resetAllButton, SIGNAL(clicked()), this, SLOT(resetAllSettings()));
    connect(settingsUi.disableLinkingCheckbox, SIGNAL(stateChanged(int)), this,
            SLOT(enableOrDisableLinkingEdit(int)));
    connect(settingsUi.runInCurrentDirectoryCheckbox, SIGNAL(stateChanged(int)), this,
            SLOT(enableOrDisableObjectFilenameLabel(int)));
    ...
    colorButtons <<
        ...
        settingsUi.backgroundColorButton << settingsUi.lineNumberPanelColorButton
                 << settingsUi.fontColorButton << settingsUi.currentLineColorButton
                 << settingsUi.debugLineColorButton
                 << settingsUi.lineNumberFontColorButton;

    defaultColors <<
        //! According to colorButtons
        QColor(Qt::blue) << QColor(153, 0, 204) << QColor(255, 122, 0)
                  << QColor(0, 128, 255) << QColor(128, 0, 0) << QColor(Qt::darkGreen)
                  << QColor(Qt::darkCyan) << QColor(Qt::blue) << QColor(128, 128, 128)
                  ...
                  << QPalette().color(QPalette::WindowText) << QColor(232, 232, 255)
                  << QColor(235, 200, 40) << QColor(QPalette::WindowText);
    ...
    connect(fontsSignalMapper, SIGNAL(mapped(QWidget*)), this,
            SLOT(changeHighlightingFont(QWidget*)));

    connect(settingsUi.currentLineCheckBox, SIGNAL(clicked(bool)), this,
            SLOT(changeHighlightingLineMode(bool)));
  ...
  settingsUi.fontComboBox->setCurrentFont(
      QFont(settings.value("fontfamily", QString("Courier New")).value<QString>()));
  ...
  settingsUi.insertDebugStringCheckBox->setChecked(
      settings.value("insertdebugstring", true).toBool());
  ...
  settingsUi.currentLineCheckBox->setChecked(
      settings.value("currentlinemode", true).toBool());
  settingsWindow->show();
}
...
//! Else it is Linux
    ...
    connect(settingsUi.x86RadioButton, SIGNAL(toggled(bool)), this,
            SLOT(changeMode(bool)));
  ...
  settingsUi.assemblyOptionsEdit->setText(
      settings.value("assemblyoptions", options).toString());

  QString linkerOptions = assembler->getLinkerOptions();
  settingsUi.linkingOptionsEdit->setText(
      settings.value("linkingoptions", linkerOptions).toString());

  settingsUi.objectFileNameEdit->setText(
      settings.value("objectfilename", "program.o").toString());

  settingsUi.disableLinkingCheckbox->setChecked(
      settings.value("disablelinking", false).toBool());

  settingsUi.runInCurrentDirectoryCheckbox->setChecked(
      settings.value("currentdir", false).toBool());

  QString assemblerPath = assembler->getAssemblerPath();
  settingsUi.assemblerPathEdit->setText(
      settings.value("assemblerpath", assemblerPath).toString());

#ifdef Q_OS_WIN32
  settingsUi.linkerPathEdit->setText(
      settings.value("linkerpath", Common::applicationDataPath() + "/MinGW/bin/gcc.exe")
          .toString());
#else
  settingsUi.linkerPathEdit->setText(settings.value("linkerpath", "gcc").toString());
#endif

  disconnect(settingsUi.x86RadioButton, SIGNAL(toggled(bool)), this,
             SLOT(changeMode(bool)));
  //! Mode
  if (assembler->isx86()) settingsUi.x86RadioButton->setChecked(true);
  else
...
void MainWindow::printMasmInfo() {
  settingsUi.infoLabel->setText(
      tr("Please set path to MASM assembler (ml.exe) and linker (link.exe) on your "
         "computer ") +
...
void MainWindow::changeMode(bool x86) {
  if (x86) settings.setValue("mode", QString("x86"));
  else
...
void MainWindow::recreateAssembler(bool start) {
  ...
  bool x86 = true;
  if (settings.value("mode", QString("x86")).toString() != "x86") x86 = false;
  /*!
   *
   *  Assembler Dependent Options Begin
   *
...
void MainWindow::backupSettings() {
  ...
  backupAssemblerOptions =
      settings.value("assemblyoptions", assembler->getAssemblerOptions()).toString();
  backupAssemblerPath =
      settings.value("assemblerpath", assembler->getAssemblerPath()).toString();
  backupLinkerOptions =
      settings.value("linkingoptions", assembler->getLinkerOptions()).toString();
...
void MainWindow::saveSettings() {
  ...
  settings.setValue("insertdebugstring",
                    settingsUi.insertDebugStringCheckBox->isChecked());
...
void MainWindow::pickColor(QWidget* button, bool init) {
  ...
  if (!init) {
    QColor color =
        QColorDialog::getColor(settings.value(name, colorsMap[name]).value<QColor>());
    if (color.isValid()) {
