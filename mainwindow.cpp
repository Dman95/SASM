...
void
MainWindow::changeCurrentSavedState(bool changed) {
	if (changed)
...
void
MainWindow::setCurrentTabName(const QString& filePath, int index) {
...
bool
MainWindow::saveAsFile(int index) {
...
bool
MainWindow::deleteTab(int index, bool saveFileName) {
...
bool
MainWindow::okToContinue(int index) {
	Tab* deletingTab;
	if (index == -1)
...
void
MainWindow::changeCurrentTab(int index) {
	//set highlighter
	if (tabs->count() == 0)
		return;
...
void
MainWindow::buildProgram(bool debugMode) {
	programIsBuilded = false;
...
void
MainWindow::printLogWithTime(const QString& message, const QColor& color) {
...
void
MainWindow::printLog(const QString& message, const QColor& color) {
...
void
MainWindow::setProgramBuildedFlagToFalse() {
	programIsBuilded = false;
}

void
MainWindow::printOutput(QString msg, int index) {
	Tab* tab = 0;
	if (index != -1) {
		tab = (Tab *) tabs->widget(index);
	}
...
void
MainWindow::changeDebugActionToStart() {
	debugAction->setText(tr("Continue"));
	debugAction->setIcon(QIcon(":/images/continue.png"));
	debugAction->setShortcut(debugKey);
...
void
MainWindow::disableDebugActions(bool start) {
...
void
MainWindow::debugToggleBreakpoint() {
	CodeEditor* code = ((Tab *) tabs->currentWidget())->code;
	code->setBreakpointOnCurrentLine();
}

void
MainWindow::debugNextNi() {
	debugNextNiAction->setEnabled(false);
...
void
MainWindow::saveWatches(DebugTableWidget* table) {
	watches.clear();
...
void
MainWindow::setShowMemoryToUnchecked() {
	debugShowMemoryAction->setChecked(false);
}

void
MainWindow::setShowMemoryToChecked(const RuQPlainTextEdit::Watch& variable) {
	if (!debugShowMemoryAction->isChecked()) {
...
void
MainWindow::setShowRegistersToUnchecked() {
	debugShowRegistersAction->setChecked(false);
}

void
MainWindow::debugExit() {
...
void
MainWindow::showAnyCommandWidget() {
...
void
MainWindow::closeAnyCommandWidget() {
...
void
MainWindow::debugRunCommand(QString command, bool print) {
	static SignalLocker locker;
	if (debugger->isStopped() && locker.tryLock()) {
...
void
MainWindow::restorePrevSession(bool notNotify) {
...
void
MainWindow::initAssemblerSettings(bool firstOpening) {
	if (firstOpening) {
		/******************************************************************************
...
void
MainWindow::changeMode(bool x86) {
	if (x86)
...
void
MainWindow::recreateAssembler(bool start) {
	if (assembler) {
		delete assembler;
		assembler = 0;
	}
...
void
MainWindow::restoreSettingsAndExit() {
...
void
MainWindow::recreateHighlighter() {
	if (highlighter) {
		delete highlighter;
		highlighter = new Highlighter(assembler);
	}
...
void
MainWindow::pickColor(QWidget* button, bool init) {
...
void
MainWindow::changeHighlightingFont(QWidget* box, bool init) {
...
void
MainWindow::changeHighlightingLineMode(bool mode) {
	settings.setValue("currentlinemode", mode);
}

void
MainWindow::exitSettings() {
	saveSettings();
	settingsWindow->close();
}

void
MainWindow::resetAllSettings() {
...
void
MainWindow::changeActionsState(int widgetIndex) {
...
bool
MainWindow::removeDirRecuresively(const QString& dirName) {
	bool result = true;
	QDir dir(dirName);
