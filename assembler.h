...
class Assembler : public QObject {
 ...
 public:
  ...
  struct LineNum {
    ...
    quint64 numInMem;
    bool operator==(const LineNum& ln) { return ln.numInCode == numInCode; }
  };
  /*!
  ...
  /*!
   * \brief Parses the listing file
   * \a lst and fills vector
   * \a lines with results of parsing
   * \param lst listing file
   * \param programFilePath path to the main .asm file
   * \param lines vector with info to fill
   * \param offset difference between address of instruction in object
   * file and address of instruction in memory
   */
  virtual void parseLstFile(QFile& lst, QString& programFilePath,
                            QVector<Assembler::LineNum>& lines, quint64 offset) = 0;
  ...
  virtual void fillHighligherRules(
      ...
      QRegExp& commentStartExpression, QRegExp& commentEndExpression) = 0;
 ...
 *
 * Below you will find what and where to modify appropriate header and source files when
 * you want to add a feature.
 *
 * \section section1 Commenting
 *
 * When commenting use doxygen's syntax. Place the comment above the intended line. After
 * adding the feature, run doxygen.exe configfile to update the documentation. Be sure to
 * double check the documentation to ensure the added comments were parsed correctly.
 *
 * \section section2 Adding Assembler Support
 *
 * Adding support is a relatively straight forward process. Each supported assembler is a
 * derived Assembler and has its own header and cpp file.
 *
 * \subsection step1 Step 1: Creating the header and cpp
 *
 * The first step is to create the new header and cpp file for the assembler. These should
 * be named all lowercase without gaps
 *
 * \subsection step2 Step 2: Creating the assembler class
 *
 * The decleration of the class may best be discussed in light of already supported
 * assemblers. Take for example, NASM. The NASM class is defined as: class NASM : public
 * Assembler
 *
 ...
 * If you are unsure what to add, refer to the already supported assemler classes. You may
 * copy and paste them.
 *
 * \subsection step3 Step 3: Adding it to the Build Options
 *
 * You should hopefully know enough QT to be able to add form controls. The code for
 * modifying the build menu can be found in mainwindow.cpp. Refer to its documentation for
 * more reference on where to add/modify code.
 *
 ...
 *
 * The help file, helpENG.html, needs to be translated and saved into a new help<NEW
 * LANGUAGE>.html file. The void MainWindow::openHelp() must be modified to support the
 * added language. This is done by adding another if statement.
 *
 * \subsection interface Interface language
 *
 * New language_<new language>.ts file needs to be created and filled with translations
 * using Qt Linguist. Compiled language_<new language>.qm and qt_<new_language>.qm files
 * need to be added in SASM folder. Support of new language needs to be added in settings
 * (mainwindow.cpp) and in loading code (main.cpp).
 */
