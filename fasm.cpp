...
FASM::FASM(bool x86, QObject* parent) : GccBasedAssembler(x86, parent) {}
...
void FASM::parseLstFile(QFile& lstOut, QString& programFilePath,
  ...
  while (!lstStream.atEnd()) {
    ...
    int k = 0;
    while (k < second.length() && second[k].isSpace()) k++;
    second.remove(0, k);
  ...
  if (index != -1) {
    ...
    if (isx86()) {
      if (code->toPlainText().indexOf(
              QRegExp("\\s+[Mm][Oo][Vv] +[Ee][Bb][Pp] *, *[Ee][Ss][Pp]"), index + 1) !=
          index + 1) {
      ...
      if (code->toPlainText().indexOf(
              QRegExp("\\s+[Mm][Oo][Vv] +[Rr][Bb][Pp] *, *[Rr][Ss][Pp]"), index + 1) !=
          index + 1) {
        QTextCursor cursor = code->textCursor();
        cursor.movePosition(QTextCursor::Start);
        cursor.movePosition(QTextCursor::Right, QTextCursor::MoveAnchor, index + 1);
...
QString FASM::getAssemblerOptions() { return "$SOURCE$ $PROGRAM.OBJ$ -s $LSTOUTPUT$"; }
