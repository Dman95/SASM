...
QString
FASM::getListingFilePath(QFile& lstOut) {
...
quint64
FASM::getMainOffset(QFile& lstOut, QString entryLabel) {
...
void
FASM::parseLstFile(QFile& lstOut, QVector<Assembler::LineNum>& lines, quint64 offset) {
	QFile lst(getListingFilePath(lstOut));
	lst.open(QFile::ReadOnly);
...
void
FASM::putDebugString(CodeEditor* code) {
...
QString
FASM::getAssemblerOptions() {
	return "$SOURCE$ $PROGRAM.OBJ$ -s $LSTOUTPUT$";
}

QString
FASM::getLinkerOptions() {
	QString options;
	if (isx86())
