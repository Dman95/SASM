...
int main(int argc, char *argv[])
{
    ...
    if (! settings.contains("language")) { //language choosing
        ...
        QPushButton *spaButton = msgBox.addButton(QString("Español"), QMessageBox::NoRole);
        msgBox.setWindowTitle(QString("Choose language"));
        msgBox.setText(QString("Choose language / Выберите язык / Dil seçin / 选择你的语言 / Sprache auswählen / Scegli lingua / Elija su idioma"));
        msgBox.exec();
        ...
        } else if (msgBox.clickedButton() == spaButton) {
    ...
    if (settings.value("language", 0).toInt() == 0) { //russian language
        ...
        a.installTranslator(&qtTranslator);
    } else if (settings.value("language", 0).toInt() == 6) { //spanish language
