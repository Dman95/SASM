...
Debugger::Debugger(QTextEdit* tEdit, const QString& exePathParam,
                   const QString& lstPathParam, const QString& programPathParam,
                   const QString& workingDirectoryPathParam,
                   const QString& inputPathParam, Assembler* assembler, QWidget* parent)
    : QObject(parent) {
  ...
  lstPath = lstPathParam;
  programPath = programPathParam;
  ...
  // process->setWorkingDirectory(workingDirectoryPath);
  ...
  QObject::connect(bufferTimer, SIGNAL(timeout()), this, SLOT(processOutput()),
                   Qt::QueuedConnection);
...
void Debugger::processOutput() {
  ...
  if (index != -1) {
    // if whole message ready to processing (end of whole message is "(gdb)")
...
void Debugger::processMessage(QString output, QString error) {
  ...
  if (c == 0) {  // in start wait for printing of start gdb text like this:
    ...
    C:\Users\Dmitri\Dropbox\Projects\SASMstatic\release\Program\SASMprog.exe... done.
    (gdb)*/
    c++;
  ...
  if (dbgSymbols) {  // debug symbols exists
    ...
    if (c == 2 && output.indexOf(QString(" in ")) != -1) {
      ...
      // take offset in hexadecimal representation (10 symbols) from string and convert it
      // to int
    ...
    if ((c == 2 && output.indexOf(QString(
                       "Make breakpoint pending on future shared library load")) != -1) ||
        (c == 1 && output == " ")) {
      actionTypeQueue.enqueue(anyAction);
      processAction(
          tr("An error has occurred in the debugger. Please check the names of the "
             "sections."));
      QObject::disconnect(process, SIGNAL(readyReadStandardOutput()), this,
                          SLOT(readOutputToBuffer()));
      emit finished();
    }
...
void Debugger::processAction(QString output, QString error) {
  ...
  if (output.indexOf(QRegExp(cExitMessage)) != -1) {  // if debug finished
    ...
    if (continuingIndex != -1) {
      ...
      if (outputLength > 0) {
        QString msg = output.mid(continuingIndex + continuingLength,
                                 outputLength);  // program output
  ...
  if (actionType == breakpoint) return;
  ...
  if (actionType == si || actionType == ni || actionType == showLine) {
    ...
    if (!found) {
      ...
      // output = QString::number(lineNumber) + tr(" line") +
      // output.mid(output.indexOf("()") + 2);
      if (actionType == showLine) return;
  ...
  if (actionType == anyAction) {
    ...
    // process as ni or si
    if (output.indexOf(QRegExp("0x[0-9a-fA-F]{8,16} in ")) != -1 && !backtrace) {
    ...
    if (output.indexOf(QString("No symbol")) == -1 &&
      ...
      if (index == -1) isValid = false;
        ...
        for (int i = output.size() - 1; i >= 0; i--) {
          if (output[i].isSpace()) output.remove(i, 1);
        }
      }
    }
    memoryInfo info;
    if (isValid) info.value = output;
  ...
  if (actionType == infoRegisters) {
    ...
    for (int i = 0; !registersStream.atEnd(); i++) {
      ...
      if (general.contains(info.name) || segment.contains(info.name) ||
          fpu_info.contains(info.name) || info.name == ip || flags.contains(info.name) ||
          fpu_stack.exactMatch(info.name)) {
        registersStream >> info.hexValue;
        registersStream.skipWhiteSpace();
        info.decValue = registersStream.readLine();
      } else if (mmx.exactMatch(info.name) || xmm.exactMatch(info.name) ||
                 ymm.exactMatch(info.name)) {
        ...
        if (mmx.exactMatch(info.name)) info.decValue = fields["v8_int8"];
...
bool Debugger::isStopped() { return stopped; }
...
void Debugger::doInput(QString command, DebugActionType actionType) {
  if (actionType != none) actionTypeQueue.enqueue(actionType);
  // put \n after commands!
  if (actionType == ni || actionType == si || actionType == showLine) {
    emit highlightLine(-2);  // does not turn off debug mode
    stopped = false;
  }
  if (process) process->write(command.toLatin1());
}

void Debugger::setWatchesCount(int count) { watchesCount = count; }

void Debugger::processLst() {
  // set accordance with .lst file and program in memory
  QFile lst;
  lst.setFileName(lstPath);
  if (lst.open(QIODevice::ReadOnly)) {
    // heuristic:
    // if debug symbols exists - offset is difference between main labels in listing and
    // in executable otherwise - offset is difference between entry point, labeled as
    // "start", in listing and in executable
    quint64 mainOffset;
    if (dbgSymbols) mainOffset = assembler->getMainOffset(lst, "main");
    else
    ...
    if ((qint64) mainOffset == -1) {
      actionTypeQueue.enqueue(anyAction);
      processAction(
          tr("Entry point was not found! Entry point should have label \"main\" (if gcc "
             "linker is used) or \"start\" (otherwise)."));
      QObject::disconnect(process, SIGNAL(readyReadStandardOutput()), this,
                          SLOT(readOutputToBuffer()));
      emit finished();
      return;
    ...
    assembler->parseLstFile(lst, programPath, lines, offset);
    lst.close();
    if (lines.isEmpty()) {
      actionTypeQueue.enqueue(anyAction);
      processAction(tr("Executable sections were not found! ") +
                    tr("For correct debugging executable sections should have name "
                       "\".text\" or \".code\" (for MASM only)."));
      QObject::disconnect(process, SIGNAL(readyReadStandardOutput()), this,
                          SLOT(readOutputToBuffer()));
      emit finished();
      return;
...
void Debugger::run() {
  ...
  if (inputFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
    ...
    for (auto&& arg : in.readAll().split(' ')) runCommand.append(' ').append(arg);
  }
  runCommand.append('\n');

  doInput(QString("cd " + workingDirectoryPath + "\n"), none);
  doInput(QString(runCommand), none);
...
void Debugger::changeBreakpoint(quint64 lineNumber, bool isAdded) {
  ...
  foreach (pair, breakPairs) {
    if (pair.numInMem == numInMem) count++;
  }
  ...
  if (wasStarted) emit needToContinue();
}
