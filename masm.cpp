...
MASM::MASM(bool x86, QObject* parent) : Assembler(x86, parent) {
#ifndef Q_OS_WIN32
  QMessageBox::critical(0, tr("Error!"), tr("MASM is available on Windows only."));
#else
  if (!x86) {
    QMessageBox::critical(0, tr("Error!"),
                          tr("Only 32-bit version of MASM is available."));
  }
#endif
}

QString MASM::getAssemblerPath() { return "C:/masm32/bin/ml.exe"; }

QString MASM::getLinkerPath() { return "C:/masm32/bin/link.exe"; }
...
quint64 MASM::getMainOffset(QFile& lst, QString entryLabel) {
  ...
  while (!lstStream.atEnd()) {
    ...
    if (empty1.exactMatch(line)) continue;
    if (empty2.exactMatch(line)) continue;
    if (!mainLabel.exactMatch(line)) continue;
...
void MASM::parseLstFile(QFile& lst, QString& programFilePath,
  ...
  while (!lstStream.atEnd()) {
    ...
    if (inTextSection) {
      ...
      if (line.contains("invoke", Qt::CaseInsensitive)) {
        ...
        if (macro.exactMatch(macroInstruction)) continue;
        int k = 0;
        while (k < macroInstruction.length() && macroInstruction[k].isSpace()) k++;
        macroInstruction.remove(0, k);
        if (macroInstruction[0] == '=') continue;
      ...
      if (line.startsWith("\t\t\t\t")) {
        mayBeMacro = line;
        int k = 0;
        while (k < mayBeMacro.length() && mayBeMacro[k].isSpace()) k++;
      ...
      if (!stringWithCode.exactMatch(line) || macro.exactMatch(line) ||
          sscanf(s, "%llx", &a) != 1)
        continue;
      ...
      int k = 0;
      while (k < line.length() && line[k].isSpace()) k++;
      line.remove(0, k);
      if (line[0] == '*') continue;
